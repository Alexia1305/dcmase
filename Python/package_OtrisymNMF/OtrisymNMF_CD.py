...
def OtrisymNMF_CD(X, r, numTrials=1, maxiter=1000, delta=1e-7, time_limit=300, init_method=None, verbosity=1,
                  init_seed=None, v_init = None):
    """
    ...
    for trial in range(numTrials):
        ...
        # SVCA or SSPA
            ...
            S = update_S(X, r, w, v)
            nmi_after_init = normalized_mutual_info_score(v_init, v)
        ...
        for iteration in range(maxiter):
            ...
            prev_error = error
            if normX > np.linalg.norm(S, 'fro'):
                error = compute_error(normX, S)
            else:
                W = np.zeros((n, r))
                for i in range(n):
                    W[i, v[i]] = w[i]
                error = np.sqrt((np.linalg.norm((X- (W@S)@W.T), 'fro'))**2)/normX
    ...
    return w_best, v_best, S_best, error_best, nmi_after_init
...
def cardan(a, b, c, d):
    ...
    if delta < 0:
        ...
        z1 = 2 * math.sqrt(val) * math.cos(phi / 3)
        z2 = 2 * math.sqrt(val) * math.cos((phi + 2 * math.pi) / 3)
        z3 = 2 * math.sqrt(val) * math.cos((phi + 4 * math.pi) / 3)
